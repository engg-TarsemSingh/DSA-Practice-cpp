...
class graph{
    ...
    void searchin(unordered_map<char,bool>& visited , char starting){
        cout<<starting<<" ";
    ...
    void Deapth_first_search(char starting){
        unordered_map<char,bool> visited;
        for(auto i : adjlist){
            visited[i.first] = false;
        }

        searchin(visited,starting);
        for(auto i : visited){
            if(i.second==false){
                searchin(visited,i.first);
            }
        }
    }
};
...
int main(){
    ...
    cin>>starting;
    g1.Deapth_first_search(starting);
